...
clear index
index = true(size(T,1),1);
T = sortrows(T,{'takeoff'});
for i = 1:(size(T,1)-1)
    if (T.takeoff(i+1) - T.takeoff(i)) < duration(0,0,1)
        index(i) = false; 
    end
end
T.takeoff(~index) = NaT;
...
clear a rng
% Setup
m = size(T,1);              % this will look for at most m landing
vm = zeros(m,1);
for i = 1:m
    vm(i) = mink(data.v( (data.t > toff(i)) & ...
        (data.t < (toff(i) + duration(0,0,2)) )), 1);
end
